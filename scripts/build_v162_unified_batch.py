#!/usr/bin/env python3
"""
Build a Tessera-style audit batch for the v1.6.2 UNIFIED PIPELINE day.
...
------
  ...
  Leaf 5 :: ENGINE CODE FINGERPRINT (sha256 of legalResponsibilityScorer.ts to lock the v1.6.2 fix)
...
OUT     = ANCHORS / "2026-05-16-v1.6.2-unified-pipeline.json"
KEY     = Path.home() / ".causallayer-secrets/cert.private.pem"

UNIFIED_DIR     = Path("/home/ubuntu/calb2/v09/v162_unified")
...
def main():
    ...
    headline = {
        "engine_version_tag": "v1.6.2 (legal scorer) + v3.1.0 (engine package)",
    ...
    leaves.append({
        ...
        "title": "v1.6.2 unified pipeline — run summary (53 engines × 280 cases)",
    ...
    # Leaf 5: code fingerprint of the legal scorer (locks the v1.6.2 fix)
    scorer_sha = sha256_file(LEGAL_SCORER)
    leaves.append({
        "leaf_index": 5,
        "leaf_type": "engine_code_fingerprint",
        "title": "legalResponsibilityScorer.ts (v1.6.2) source fingerprint",
        "artefact_path": str(LEGAL_SCORER),
        "sha256": scorer_sha,
        "scope": "Locks the v1.6.2 fix: in-house + sanctioned_role=joint → deployer (L5-903 Ascend Ecom).",
        "supporting_artefacts": {
            "orchestrator": {"path": str(ORCHESTRATOR), "sha256": sha256_file(ORCHESTRATOR)},
            "scope_check_script": {"path": str(SCOPE_SCRIPT), "sha256": sha256_file(SCOPE_SCRIPT) if SCOPE_SCRIPT.exists() else None},
        },
    })
    ...
    batch = {
        ...
        "title": "CausalLayer v1.6.2 — UNIFIED MULTI-ENGINE PIPELINE DAY (53 engines × 280 cases)",
